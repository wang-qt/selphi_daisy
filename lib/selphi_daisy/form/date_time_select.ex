defmodule SelphiDaisy.Form.DateTimeSelect do
  @moduledoc """
  DateSelect,通过 年/月/日 时:分:秒 select实现日期/时间选择的组件
  """
  use  SelphiDaisy.Component
  import SelphiDaisy.Form.Utils
  alias Surface.Components.Form.{Field, FieldContext,DateTimeSelect, ErrorTag, Label}
  ...
  def render_control(assigns, hg_size ) do
    ...
    builder = fn b ->
      ...
      {#for {item, index} <- Enum.with_index(~w/year month day hour minute/a)}
        ...
        {#if @separators[item] }
          <span class="px-1 hidden sm:inline-flex flex-none" >{@separators[item]}</span>
        {/if}
      {/for}
      """
    end
    ...
    <FieldContext  name={@field} >
      <div class={
        [
          "control flex flex-col sm:flex-row sm:items-center gap-1 ",
