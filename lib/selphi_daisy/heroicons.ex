defmodule SelphiDaisy.Heroicons do
  @moduledoc """
  heroicons svg icon component.
  使用 `icons/heroicons-outline-sprite.svg` 和 `icons/heroicons-solid-sprite.svg`
  ...
  @doc "图标名称,参见 https://heroicons.dev/ 获取图标名称"
  ...
  def render(assigns) do
    ...
    <svg xmlns="http://www.w3.org/2000/svg"
      class={ ["inline-block"] ++
       ...
       {!--type为stroke才填 fill="none" --}
       {...set_fill_prop(assigns)}
      viewBox="0 0 24 24"
    >
       {#if  @type == "stroke" }
          <use href={"/icons/heroicons-outline-sprite.svg##{@name}"} />
       {#else}
          <use href={"/icons/heroicons-solid-sprite.svg##{@name}"} />
       {/if}
    </svg>
    """
  end
