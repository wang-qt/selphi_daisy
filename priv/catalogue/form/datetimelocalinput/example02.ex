defmodule SelphiDaisy.Form.DateTimeLocalInput.Example02 do
  @moduledoc """
  ...
  def render(assigns) do
    ...
    <Form
      ...
      <DateTimeLocalInput  field={:datetimelocal1  }
          ...
          size="xs" icon_left="clock" />

      {!-- step默认为60秒，不显示描述。step<60 显示秒数 --}
      <DateTimeLocalInput  field={:datetimelocal2  }
          value={ @datetime }  step=1
           label="日期"  help_text="请选择日期时间"
           bordered  color="info" control_class="w-64"
          size="sm" icon_left="clock" />

      {!-- step默认为60秒，不显示描述。step<60 显示秒数 --}
      <DateTimeLocalInput  field={:datetimelocal3  }
          value={ @datetime }  step=1
           label="日期"  help_text="请选择日期时间"
           bordered  color="warning" control_class="w-64"
          size="md" icon_left="clock" />
